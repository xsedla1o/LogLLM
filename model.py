...
class LogLLM(nn.Module):
    ...
    def train_helper(self, inputs, seq_positions, labels):
        ...
        if type(self.Llama_model) == peft.peft_model.PeftModelForCausalLM:
            ...
            answer_embeddings = self.Llama_model.model.model.embed_tokens(
                answer_tokens_ids
            )
        else:
        ...
        ins2 = instruc_embeddings[1][self.instruc_tokens["attention_mask"][1].bool()][
            1:
        ]
        ...
        for seq_embedding, answer_embedding, answer_tokens_att in zip(
            ...
            full_prompt_embedding = torch.cat(
                [ins1, seq_embedding, ins2, answer_embedding[answer_tokens_att]]
            )
    ...
    def forward(self, inputs, seq_positions):
        ...
        answer_prefix_tokens = self.Llama_tokenizer(
            prefix, padding=True, return_tensors="pt"
        )["input_ids"][0, 1:].to(self.device)

        if type(self.Llama_model) == peft.peft_model.PeftModelForCausalLM:
            instruc_embeddings = self.Llama_model.model.model.embed_tokens(
                self.instruc_tokens["input_ids"]
            )
            answer_prefix_tokens_embeddings = self.Llama_model.model.model.embed_tokens(
            ...
            answer_prefix_tokens_embeddings = self.Llama_model.model.embed_tokens(
                answer_prefix_tokens
            )
        ...
        for seq_embedding in seq_embeddings:
            prompt_embedding = torch.cat(
                [ins1, seq_embedding, ins2, answer_prefix_tokens_embeddings]
            )
        ...
        eos_token_id_tensor = (
            torch.tensor(eos_token_id).to(self.device)
        ...
        while not this_peer_finished:
            ...
            if isinstance(self.Llama_model, peft.peft_model.PeftModelForCausalLM):
                next_tokens_embeddings = self.Llama_model.model.model.embed_tokens(
                    next_tokens
                )
            else:
                next_tokens_embeddings = self.Llama_model.model.embed_tokens(
                    next_tokens
                )

            # update attention_mask
            attention_mask = torch.cat(
                [attention_mask, unfinished_sequences[:, None]], dim=1
            )
