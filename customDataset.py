...
class CustomDataset(Dataset):
    def __init__(self, file_path, drop_duplicates=False):
        ...
        print(
            "Number of normal samples in original dataset: {}".format(
                (df["Label"].values == 0).sum()
            )
        )
        print(
            "Number of anomalous samples in original dataset: {}".format(
                (df["Label"].values == 1).sum()
            )
        )
        ...
        self.sequences = np.array(
            [content.split(" ;-; ") for content in contents], dtype=object
        )
        self.labels = df["Label"].values
        if drop_duplicates:
            print(
                "Number of normal samples after dropping duplicates: {}".format(
                    (self.labels == 0).sum()
                )
            )
            print(
                "Number of anomalous samples after dropping duplicates: {}".format(
                    (self.labels == 1).sum()
                )
            )
...
class BalancedSampler(Sampler):
    def __init__(self, dataset, target_ratio=0.3, max_samples=None, min_samples=50000):
        ...
        self.min_samples = (
            min_samples  # only if max_samples is None, min_samples can work
        )
        ...
        self.majority_indices = (
            self.normal_indices
            if self.minority_indices is self.anomalous_indices
            else self.anomalous_indices
        )

        self.minority_count = max(
            int(
                (self.target_ratio * len(self.majority_indices))
                / (1 - self.target_ratio)
            ),
    ...
    def __iter__(self):
        oversampled_minority = np.tile(
            self.minority_indices, int(self.minority_count / len(self.minority_indices))
        )
        ...
        combined_indices = np.concatenate(
            [self.majority_indices, oversampled_minority, oversampled_minority_]
        )
        if len(combined_indices) > self.total_size:
            combined_indices = np.random.choice(
                combined_indices, self.total_size, replace=False
            )
        else:
            combined_indices = np.tile(
                combined_indices, int(self.total_size / len(combined_indices))
            )
            combined_indices_ = np.random.choice(
                combined_indices, self.total_size - len(combined_indices), replace=False
            )
